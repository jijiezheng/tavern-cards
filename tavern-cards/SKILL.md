...
帮助用户创建和编辑 SillyTavern 角色卡与世界书的内容和配置。覆盖角色人设编写、世界观构建、MVU 变量系统、EJS 动态方案、条目配置、开场白创作等全流程。聚焦于内容创作和流程编排，涉及 schema.ts、EJS 模板以及 MVU 的酒馆助手脚本，但不涉及其他酒馆助手脚本的编写。
...
10. **UI 界面开发**（如使用 MVU）→ `references/ui/`
    - 纯文本状态栏（`text.md`）：直接编辑 `正则/状态栏界面.html`，无需二次打包
    - 前端状态栏（`frontend.md`）：先打包预览版本对接本地 dev server，部署后再打包生产版本
    - 如需新增其他前端界面（战斗界面、开局选择等），先阅读 `regex-scripts.md` 理解正则配置规则，再参考 `assets/mvu-patch.json` 中 `regex_scripts` 的既有条目写法
11. **打包输出** → `references/packaging.md`
    - 先执行打包前检查清单，再执行 `node scripts/tavern-cards-forge.mjs pack {project}`
